...
ipcMain.on('get-serial-ports', (event: IpcMainEvent, ipcCallBackEvent: string) => {
...
ipcMain.on("change-baudrate", (event: IpcMainEvent, is_firmware_update: boolean) => {
...
ipcMain.on('export-project', async (event, data: ArrayBuffer, fileName: string) => {
    try {
...
autoUpdater.on('update-downloaded', (info: UpdateDownloadedEvent) => {
    const notes = Array.isArray(info.releaseNotes) ? info.releaseNotes.map(n => (typeof n === 'string' ? n : n.note)).join('\n') : (info.releaseNotes || '');
    const message = process.platform === 'win32' ? notes : (info.releaseName ?? `Version ${info.version}`);
